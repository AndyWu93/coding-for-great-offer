 ...
 *
 * 题意：求两个有序数组的整体中位数 （难，但是超级经典，据说每年都出现）
 * 本题思路：
 * arr1，arr2均为有序数组
 ...
 *
 * 	2.基于getUpMedian函数，实现 findKthNum(arr1,arr2,k):返回arr1，arr2整体第k小的数
 ...
 *			如何淘汰？
 *			假设arr1[0]>arr2[n2-1],arr1前面的一直到k-arr2.length的位置都淘汰了
 *			假设arr2[0]>arr1[n1-1],arr2前面的一直到k-arr1.length的位置都淘汰了
 *			这时假设arr1一共淘汰了x个数，arr2一共淘汰了y个数，
 *			arr1arr2剩下的数一样多。是不是直接求剩下的数的中位数呢？
 *			不是，arr1arr2需要各自再淘汰一个数，因为x+y+剩下的数的中位数，刚好是第k-1个，不是第k个
 *			arr1arr2如何各自再淘汰一个数？
 *			arr1看第x+1个数，和arr2[n2-1]比较，如果大于等于arr2[n2-1]，那第x+1个数就是结果，否则淘汰掉
 *			arr2看第y+1个数，和arr1[n1-1]比较，如果大于等于arr1[n1-1]，那第y+1个数就是结果，否则淘汰掉
...
public class Code03_FindKthMinNumber {
	...
	* */
	public static int findKthNum(int[] arr1, int[] arr2, int kth) {
		...
		int l = longs.length;
		int s = shorts.length;
		/*1. k小于等于短数组长度，直接拿各自前k个数调用getUpMedian*/
		if (kth <= s) { // 1)
		...
		* 如何淘汰？
		* 极限假设法，
		* 假设短数组里的数都比长数组大，那kth就在long数组的kth-s-1位置，如果这个数比短数组的最大值（最后一个值）大或相等，那这个数就是kth
		* 同理，假设长数组里的数都比短数组大，确认kth-l-1位置
		* 如果以上两者都不是，表明kth在两个数组剩下的没有看的位置里，调用算法可得
		* */
		if (kth > l) {
